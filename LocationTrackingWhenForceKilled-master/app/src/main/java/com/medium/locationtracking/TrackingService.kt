...
class TrackingService: Service(), GoogleApiClient.ConnectionCallbacks,
    ...
    private val locationListener: LocationListener = object : LocationListener {
        override fun onLocationChanged(location: Location) {
            ...
            Log.v("MediumApplati", location.latitude.toString())
    ...
    internal inner class ContinousThread : Thread() {
        ...
        private fun sendLocationToServer() {
            Log.i("Serverrrrrrrrrrr","server$currentLocation?.latitude.toString()")
         //   databaseReference?.child("Users")?.child("demo")?.setValue(currentLocation?.latitude.toString())
...
//Get a reference to the database, so your app can perform read and write operations//
                    ...
                    if (location != null) {
                        Toast.makeText(applicationContext, "s$location", Toast.LENGTH_LONG)
                            .show()
                        Log.i("sdf", location.toString())
                        //Save the location data to the database//
